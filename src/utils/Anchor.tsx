// === Rotate.tsx ===
// 개인화된 기준 좌표계 + 동적 좌표계 비교를 통한 회전 각도 계산
...
// === 2. 입술 중앙점 찾기 (Anchor.tsx 방식) ===

/**
 * Anchor.tsx의 2점 기준 시스템을 사용하여 입술 중앙점을 찾습니다.
 * @param landmarks - 얼굴 랜드마크
 * @param nosePoint - 코끝점 (기준점 A)
 * @param eyePoint - 눈 위 점 (기준점 B)
 * @returns 입술 중앙점
 */
export function findMouthCenterWithAnchor(
  landmarks: Point3D[]
): Point3D {
...
/**
 * Anchor.tsx 방식으로 입술 중앙점의 상대 위치를 계산합니다.
...
/**
 * Anchor.tsx 방식으로 상대 위치로부터 입술 중앙점을 복원합니다.
...
/**
 * 사용자의 정면 얼굴을 기준으로 개인화된 좌표계를 생성합니다.
 * Anchor.tsx 방식을 사용하여 입술 중앙점을 찾고, 이를 기준으로 좌표계를 생성합니다.
...
/**
 * 기준 좌표계와 동적 좌표계를 비교하여 회전 각도를 계산합니다.
...
/**
 * 벡터 기반과 거리 기반 방법을 결합한 하이브리드 회전 각도 계산
 * @param personal - 개인화된 기준 좌표계
 * @param dynamic - 동적 좌표계
 * @returns 하이브리드 회전 각도
 */
...
/**
 * 회전 각도로부터 입술의 깊이를 추정합니다.
 * @param rotationAngles - 회전 각도들
 * @param baseDepth - 기본 깊이 (경험적 값)
 * @returns 추정된 깊이
 */
